...
fileLogHandler = logging.FileHandler(
	'./log/' + str(datetime.datetime.now()) + '.txt', "w")
...
cookieRequest = requests.post('https://helpdesk.gridpp.rl.ac.uk/',
	verify=False, data={'user': USER_NAME, 'pass': USER_PASSWORD})
...
def isResponseExpected(response):
	if (re.search(r'((^Status:.*\n)|(^))RT\/3.8.1 200 Ok\n\n', response)
		is not None):
		if re.search(r'\n#.*', response):
			logger.warning("Response has a line with a # in it... this could be"
...
def getTicketAttachments(id):
	...
	if len(re.findall(r'\nAttachments: ', response.text)) != 1:
		raise Exception(("Should be exactly one 'Attachments: ' in the response"
			" for ticket id: ") + str(id))
		return []
...
def getTicketContent(id):
	strId = str(id)
	ticketAttachments = getTicketAttachments(id)
	if len(ticketAttachments) != 5:
		...
		raise Exception('Unexpected length of ticket attachments at id: ' +
			str(id) + ' this probably means that this ticket has been merged'
			' so only some of it would be dealt with...')
		return
	response = requests.get(firstUrlPart + 'ticket/' + strId + '/attachments/' +
		str(ticketAttachments[1]) + '/content', verify=False, cookies=cookie)
...
def getTicketContentNOKs(id):
	...
	for stringNOK in stringNOKs:
		...
		# note that the reverse split means that the comma and space are reversed
...
def getTicketsInChecksumQueue():
	response = requests.get(firstUrlPart +
		'/search/ticket?query=Queue=%27Checksum%27', verify=False,
		cookies=cookie)
...
def addAttachmentFromFile(id, filePath):
	with open(filePath, 'r') as file:
		...
		return addCommentAndAttachment(id, attachment,
			...
			comment='Attaching a test shell script using the REST api...')
...
def canResolveByTest1(ticket):
	...
	for index, nok in enumerate(ticket['NOKs']):
		...
		logger.debug(" Test1: ticket " + str(ticket['id']) + ": NOK#" +
			str(index) + " resolvable?: " + str(ticket['resolved'][index]))
...
def getStager(ticket):
	logger.debug("ssh into " +
		ticket['diskserver'] + " cat for /etc/castor/castor.conf")
	try:
...
def canResolveByTest2(ticket):
	...
	for index, nok in enumerate(ticket['NOKs']):
		...
		try:
			...
			decodedOutput = output.decode()
			parseDiskcopy = re.findall((r"(?<=DISKCOPY_STAGED      )[^:\n]*:"
				r"\/exportstage\/castor[^ ]*@[^ ]*(?= )"), decodedOutput)
			if len(parseDiskcopy) == 1 and parseDiskcopy[0] != "":
				...
				if castorPointsTo != diskServerWithPhysicalFilePath:
					ticket['results'][index][1] = ('Stager points to a different'
						' physical file path.')
					ticket['resolved'][index] = True
					ticket['action'][index] += ('(Test2) Delete the physical '
						'file. (If it even exists - check Test1 results).')
				else:
					flagCanResolveTicket = False
					...
					'File \([0-9]*\) not in ns \/ not in stager'),
				decodedOutput) is not None):
				ticket['results'][index][1] = ('Stager has no record of this '
					'logical file path.')
				ticket['resolved'][index] = True
				ticket['action'][index] += ('(Test 2) Delete the physical file.'
					' (If it even exists - check Test1 results).')

			elif (re.search(r'(\n|^)File .*not in stager\n', decodedOutput)
				is not None and re.search(r'\nFile migrated\n', decodedOutput)
				is not None and re.search(r'\nTape fileclass with \d* cop',
					decodedOutput) is not None):
				ticket['results'][index][1] = ('Stager thinks this logical file'
					' is on tape so does not point at this physical file')
				ticket['resolved'][index] = True
				ticket['action'][index] += ('(Test 2) Delete the physical file.'
					' (If it even exists - check Test1 results).')
				...
				raise Exception("couldn't understand ralreplicas command: " +
					'./sshRalreplicas.sh' + " " + stager + " " +
					logicalFilePath)
				flagCanResolveTicket = False
		logger.debug(" Test2: ticket " + str(ticket['id']) + ": NOK#" +
			str(index) + " resolvable?: " + str(ticket['resolved'][index]))
	return (flagCanResolveTicket, ticket)
...
def goThroughTicket(id):
	...
	if test1[0] and not test2[0]:
		...
		logger.warning("This is a boundary case, test1 says resolve the ticket"
			", test2 says don't resolve!")
	ticket = test2[1]
	return (flagCanResolveTicket, ticket)
...
def getResultsOfTicketTest(id):
	...
	resultOfTicket = "THIS TICKET CAN BE RESOLVED\n\n" if resultOfTests[0] else (
		"THIS TICKET CANNOT BE RESOLVED\n\n")
	ticket = resultOfTests[1]
	checkFineToResolve = True
	for index, nok in enumerate(ticket['NOKs']):
		resultOfTicket += 'NOK #' + str(index) + "\n"
		resultOfTicket += '	Logical: ' + nok['firstLink'] + "\n"
		resultOfTicket += '	Physical: ' + nok['secondLink'] + "\n"
		resultOfTicket += '	Test 1: ' + str(ticket['results'][index][0]) + "\n"
		resultOfTicket += '	Test 2: ' + str(ticket['results'][index][1]) + "\n"
		resultOfTicket += '	Action: '
		resultOfTicket += str(ticket['action'][index]) if (
			str(ticket['action'][index]) != '') else 'Needs manual investigation'
		resultOfTicket += "\n"
		resultOfTicket += '	Resolvable?: ' + str(ticket['resolved'][index]) + "\n"
		resultOfTicket += "\n"
		if ticket['resolved'][index] is False:
			checkFineToResolve = False
	if checkFineToResolve != resultOfTests[0]:
		raise Exception("The results of the test are inconsistent... watch out...")
...
listOfTicketsToTest = [174001, 174022, 174152, 174828, 174851, 174870, 174872,
	174895, 174903, 174980, 174994, 175009, 175038, 175039, 175041, 175115,
	175166, 175170, 175202, 175203, 175207, 175312, 175330, 175343, 175346,
	175347, 175349, 175350, 175369, 175370, 175371, 175373, 175417, 175470,
	175492, 175596, 175621, 175622, 175637, 175644, 175646, 175652, 175673,
	175702, 175727, 175728, 175729, 175730, 175765, 175768, 175769, 175770,
	175771, 175780, 175781, 175783, 175808, 175812, 175814, 175815, 175817,
	175818, 175823, 175832, 175834, 175851, 175852, 175861, 175863, 175866,
	175867, 175868, 175869, 175870, 175871, 175872, 175873, 175877, 175878,
	175879, 175884, 175886, 175887, 175904, 175905, 175906]
...
def TESTNOKsInAttachment():
	...
	for attach in attachments:
		response = requests.get((firstUrlPart + 'ticket/' + strId +
			'/attachments/' + str(attach) + '/content'), verify=False,
			cookies=cookie)
		NOKs = re.findall(r'(?<=\n)NOK: .*(?=\n)', response.text)
